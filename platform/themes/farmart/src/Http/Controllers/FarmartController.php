...
class FarmartController extends PublicController
{
    ...
    public function esentialajaxGetProducts(Request $request)
    {
        $products = get_products_by_collections([
                'collections' => [
                    'by' => 'id',
        ...
        $data = [];
        foreach ($products as $product) {
            $data[] = Theme::partial('ecommerce.product-essential', compact('product', 'wishlistIds'));
        }
    ...
    // new flash sale methord

    public function ajaxGetFlashSalenew(int|string $id, FlashSaleInterface $flashSaleRepository)
    {
        $flashSale = $flashSaleRepository->getModel()
            ->notExpired()
            ->where('id', $id)
        ...
        foreach ($flashSale->products as $product) {
            ...
            $data[] = Theme::partial('ecommerce.product-item-new-flashsale', compact('product', 'flashSale', 'isFlashSale', 'wishlistIds'));
        }
...
//    New featured product-meothed

    public function ajaxGetFeaturedProductsnew(Request $request)
    {
        $data = [];
        ...
        foreach ($products as $product) {
            $data[] = Theme::partial('ecommerce.product-item-featured-new', compact('product', 'wishlistIds'));
        }
...
//    Just land or new arrival section route

    public function ajaxGetProductsByCategoryIdLand(
        Request                  $request,
        ProductInterface         $productRepository,
        ProductCategoryInterface $productCategoryRepository
    )
